...
import platform.Reinforcement_Learning.LearningData.LearningData_hunter;

public class Environment_hunter extends Environment{
	...
	public LearningData getLD(Field field, Player me) {
		...
		if(mySC.condition[mySC.hunterCO] != 0){
			a = 1;
		}
	...
	public void printLDs(PrintWriter out){
		...
		+ "CO前後" + "最適戦略" + "\t" + "最適Q値" + "\t" + "平均Q値" + "\t\t"+ LearningData.getStrategysEnum(LearningData_hunter.guardStrategys));
					...
					for(int l = 0; l < lds_preCO[0][0][0].length; l++){
						for(int m = 0; m < lds_preCO[0][0][0][0].length; m++){
							...
							if(ld != null &&ld.selectedSum > VSmode.matchSum / 1000){
								if(ld.getMaxValue(ld.data_Guard) == VSmode.defaultQ_human) continue;
								out.println("\t" + i + "\t" + j + "\t" + k + "\t" + l + "\t" + m + "\t" + "CO前" + "\t"
									+ ld.getGuardOptimalStrategy() + "\t" + ld.getMaxValue(ld.data_Guard)
										+ "\t" + ld.getAvarageValue(ld.data_Guard) + "\t\t" + LearningData.getQvalues(ld.data_Guard));
							}
							if(ld2 != null && ld2.selectedSum > VSmode.matchSum / 1000){
								if(ld2.getMaxValue(ld2.data_Guard) == VSmode.defaultQ_human) continue;
								out.println("\t" + i + "\t" + j + "\t" + k + "\t" + l + "\t" + m + "\t" + "CO後" + "\t"
									+ ld2.getGuardOptimalStrategy() + "\t" + ld2.getMaxValue(ld2.data_Guard)
										+ "\t" + ld2.getAvarageValue(ld2.data_Guard) + "\t\t" + LearningData.getQvalues(ld2.data_Guard));
							}
